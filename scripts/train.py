...
from flytris.fly_player import N_OUT, TRAIN_SEED0, VALID_SEEDS, death_index, play
...
def main(argv=None):
    ...
    ap.add_argument("--groups", default=str(ROOT / "runs/readout_groups.npz"))
    ...
    out = Path(args.out or ROOT / ("runs/train" if args.player == "brain" else "runs/nobrain"))
    ...
    keep_awake()
    if args.player == "brain":
        from flytris.fly_player import FlyPlayer
        player = FlyPlayer(groups_file=args.groups, max_batch=args.max_batch)
    else:
        from flytris.fly_player import NoBrainPlayer
        player = NoBrainPlayer()
    shape = (N_OUT, player.n_features + 1)
    ...
    while gen < args.gens:
        ...
        moves = play([batch], [params], player, args.cap)[0]
        fit = fitness(batch)
        ...
        var = params[elite].var(0) + max(5 - gen / 10, 0) * NOISE_SCALE
        ...
        if (gen + 1) % args.validate_every == 0:
            ...
            play(vb, [cands] * len(vb), player, args.cap)
            ...
            if score[best] > champ_score:
                ...
                save_npz(out / "champion.npz", params=champ, validation_fitness=champ_score, gen=champ_gen,
                         validation_lines=np.array([b.lines[best] for b in vb]), groups=args.groups)
